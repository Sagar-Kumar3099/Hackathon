  ...
  register: async (userData) => {
    ...
    try {
      if (mongoose.connection.readyState === 1) {
        ...
        const storedUser = saved.toObject();
        memoryUsers.push(storedUser);
        const userToReturn = { ...storedUser };
  ...
  login: async ({ email, password }) => {
    ...
    try {
      if (mongoose.connection.readyState === 1) {
        ...
        if (userDoc) {
          let passwordMatches = await bcrypt.compare(password, userDoc.password);

          // Upgrade accounts created before password hashing was enabled.
          if (!passwordMatches && userDoc.password === password) {
            const passwordHash = await bcrypt.hash(password, 12);
            await User.updateOne({ _id: userDoc._id }, { $set: { password: passwordHash } });
    ...
    if (memUser) {
      const passwordMatches = memUser.password
